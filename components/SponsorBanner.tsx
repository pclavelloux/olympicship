...
export default function SponsorBanner() {
  ...
  return (
    <div
      className="w-full bg-base-200 border-t border-base-300 overflow-hidden py-4"
      ...
      >
        {duplicatedItems.map((item, index) => (
          <div key={`${item.type}-${index}`}>
            {item.type === 'sponsor' && item.data ? (
              renderSponsorCard(item.data, `${item.data.id}-${index}`)
            ) : (
              renderEmptySlot(`empty-${index}`)
            )}
          </div>
        ))}
      </div>
